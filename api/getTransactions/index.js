...
async function getTransakAccessToken(apiKey, apiSecret, apiUrl) {
  ...
  try {
    const response = await axios.post(url, { apiKey }, { headers: { 'api-secret': apiSecret } });
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    const transakApiUrl = process.env.TRANSAK_ENVIRONMENT === 'PRODUCTION' ? 'https://api.transak.com' : 'https://api-stg.transak.com';
    ...
    const ordersResponse = await axios.get(ordersUrl, { headers: { 'access-token': accessToken } });

    context.res = { status: 200, body: ordersResponse.data.data, headers: { 'Content-Type': 'application/json' } };
  } catch (error) {
    context.log.error("Error in getTransactions:", error.message);
    context.res = { status: 500, body: { error: `An error occurred: ${error.message}` }, headers: { 'Content-Type': 'application/json' } };
  }
};